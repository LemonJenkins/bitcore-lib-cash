...
Input.fromBufferReader = function(br, params) {
  ...
  if (params && params.hex) {
    ...
    var signatures = []
    if(params.publicKey){
      signatures = [Signature({prevTxId: input.prevTxId, outputIndex: input.outputIndex, inputIndex: 0, sigtype:Signature.SIGHASH_FORKID, publicKey: params.publicKey, signature: sigString})]
    }
